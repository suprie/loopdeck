...
- [x] Interview UI: present pre-flight questions as the existing question cards, gating the "Start run" action (2026-08-01, corrected 2026-08-01) — `RunQueuePanel`'s "Answer" (awaits `run_phase_interview`) / "Skip" (`skip_phase_interview`) buttons per pending phase, gated by a `canStart` check requiring every `queued` phase's `interview_status !== "pending"`. **Correction:** this line originally claimed `run_phase_interview` needed no new plumbing because it drove its turn through `start_fresh_and_record` "the same as any other turn" — that pipeline is non-streaming, and `claude_session.rs::answer_ask_user_question`'s own doc comment says a non-streaming (`channel: None`) `AskUserQuestion` has no UI surface and is auto-denied immediately instead of parked. So the interview's own clarifying questions could never actually reach a human. Fixed by switching to `start_fresh_and_record_streaming` with a no-op sink `Channel` (Phase 4's own trick — the channel's mere *presence* is what lets it park, not what it narrates); the parked card now genuinely surfaces through the existing tab-agnostic `StuckQuestionCallout` (`ProjectDetail.tsx`), so no bespoke question card was needed after all — just the correct pipeline underneath it
